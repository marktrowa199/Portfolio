...
import { Terminal, FileText, Menu, X, Radio } from "lucide-react";
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-[#0A192F]/90 dark:bg-[#0A192F]/90 light:bg-[#F6F7F9]/92 backdrop-blur-md border-b border-navy-700/60 dark:border-navy-700/60 light:border-[#E2E6EC] shadow-lg shadow-black/10 light:shadow-sm py-3"
          : "bg-transparent py-5"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            <div className="h-4 w-px bg-navy-700 dark:bg-navy-700 light:bg-[#E2E6EC]" />
          ...
          <div className="flex items-center gap-2 md:hidden">
            ...
            <button
              ...
              className="p-2 rounded-xl border border-navy-700 dark:border-navy-700 light:border-[#E2E6EC] light:bg-white text-slate-lightest light:text-[#111620] hover:text-cyan transition-colors"
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="fixed inset-0 top-[60px] z-40 bg-[#0A192F]/95 dark:bg-[#0A192F]/95 light:bg-[#F6F7F9]/98 backdrop-blur-xl md:hidden border-t border-navy-700/60 light:border-[#E2E6EC] flex flex-col justify-between p-6">
          <div className="space-y-6">
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-navy-800/50 dark:bg-navy-800/50 light:bg-white light:border light:border-[#E2E6EC] font-mono text-xs text-slate-muted light:text-[#525E72]">
              ...
              {NAV_ITEMS.map((item) => (
                <li key={item.href}>
                  <a
                    ...
                    className="flex items-center justify-between p-3 rounded-xl hover:bg-navy-800/60 dark:hover:bg-navy-800/60 light:hover:bg-white light:hover:shadow-sm transition-colors"
                  >
                    <span className="font-heading text-lg font-semibold text-slate-lightest dark:text-slate-lightest light:text-[#111620]">
                      {item.label}
                    </span>
      ...
      {resumeModalOpen && (
        <div
          className="fixed inset-0 z-[60] flex items-center justify-center bg-[#020817]/80 p-4 backdrop-blur-sm"
          role="presentation"
          onMouseDown={(event) => {
        ...
        >
          <section
            ...
            className="flex h-[min(88vh,760px)] w-full max-w-4xl flex-col overflow-hidden rounded-xl border border-navy-700 bg-[#0A192F] shadow-2xl"
          >
            <div className="flex items-center justify-between border-b border-navy-700 px-4 py-3 sm:px-5">
              <div>
                <h2 id="resume-modal-title" className="font-heading text-base font-semibold text-slate-lightest">
                  Resume [PDF]
                </h2>
                <p className="font-mono text-[10px] text-slate-muted">DOCUMENT PREVIEW // ARTHUR.SYS</p>
              </div>
              <button
                type="button"
                onClick={() => setResumeModalOpen(false)}
                className="rounded-lg p-2 text-slate-light hover:bg-navy-800 hover:text-cyan transition-colors"
            ...
            <div className="flex flex-wrap items-center justify-end gap-2 border-t border-navy-700 px-4 py-3 sm:px-5">
              <a
                href="/resume.pdf"
                download
                className="rounded-lg border border-navy-600 px-3 py-2 font-mono text-xs text-slate-light hover:border-cyan hover:text-cyan transition-colors"
              >
                Download PDF
              </a>
              <a
